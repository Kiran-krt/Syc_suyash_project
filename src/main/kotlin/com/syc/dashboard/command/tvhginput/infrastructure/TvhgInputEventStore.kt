...
import com.syc.dashboard.command.tvhginput.repository.jpa.TvhgInputEventStoreRepository
...
@Service
class TvhgInputEventStore @Autowired constructor(
    private val eventProducer: EventProducer,
    private val eventStoreRepository: TvhgInputEventStoreRepository,
) : EventStore {
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = TvhgInputEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw TvhgInputEventStreamNotExistInEventStoreException(
                "Could not retrieve tvhg input event stream from the event store!",
            )
        }
