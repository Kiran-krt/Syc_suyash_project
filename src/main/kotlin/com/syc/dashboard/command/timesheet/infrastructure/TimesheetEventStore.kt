...
import com.syc.dashboard.command.timesheet.exceptions.TimesheetEventStreamNotExistInEventStoreException
...
@Service
class TimesheetEventStore @Autowired constructor(
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = TimesheetEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw TimesheetEventStreamNotExistInEventStoreException(
                "Could not retrieve timesheet event stream from the event store!",
            )
        }
