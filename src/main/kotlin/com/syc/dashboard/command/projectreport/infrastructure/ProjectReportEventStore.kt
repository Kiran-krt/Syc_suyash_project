...
import com.syc.dashboard.command.projectreport.repository.jpa.ProjectReportEventStoreRepository
...
@Service
class ProjectReportEventStore @Autowired constructor(
    private val eventProducer: EventProducer,
    private val eventStoreRepository: ProjectReportEventStoreRepository,
) : EventStore {
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = ProjectReportEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw ProjectReportEventStreamNotExistInEventStoreException(
                "Could not retrieve project report from the event store!",
            )
        }
