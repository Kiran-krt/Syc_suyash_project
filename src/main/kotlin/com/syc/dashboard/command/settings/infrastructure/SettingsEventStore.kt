...
import com.syc.dashboard.command.settings.exceptions.SettingsEventStreamNotExistInEventStoreException
...
@Service
class SettingsEventStore @Autowired constructor(
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = SettingsEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw SettingsEventStreamNotExistInEventStoreException(
                "Could not retrieve settings from the event store!",
            )
        }
