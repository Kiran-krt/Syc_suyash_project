...
import com.syc.dashboard.command.expensereport.exceptions.ExpenseReportEventStreamNotExistInEventStoreException
...
@Service
class ExpenseReportEventStore @Autowired constructor(
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = ExpenseReportEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw ExpenseReportEventStreamNotExistInEventStoreException(
                "Could not retrieve expense report from the event store!",
            )
        }
