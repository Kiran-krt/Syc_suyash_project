...
import com.syc.dashboard.command.jobcode.exceptions.JobCodeEventStreamNotExistInEventStoreException
...
@Service
class JobCodeEventStore @Autowired constructor(
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = JobCodeEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw JobCodeEventStreamNotExistInEventStoreException(
                "Could not retrieve job code from the event store!",
            )
        }
