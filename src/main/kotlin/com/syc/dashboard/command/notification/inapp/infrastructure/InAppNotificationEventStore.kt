...
import com.syc.dashboard.command.notification.inapp.exceptions.InAppNotificationEventStreamNotExistInEventStoreException
import com.syc.dashboard.command.notification.inapp.repository.jpa.InAppNotificationEventStoreRepository
...
@Service
class InAppNotificationEventStore @Autowired constructor(
    private val eventProducer: EventProducer,
    private val eventStoreRepository: InAppNotificationEventStoreRepository,
) : EventStore {
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = InAppNotificationEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw InAppNotificationEventStreamNotExistInEventStoreException(
                "Could not retrieve in app notification event stream from the event store!",
            )
        }
