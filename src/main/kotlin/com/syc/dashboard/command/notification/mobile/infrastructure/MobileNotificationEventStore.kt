...
import com.syc.dashboard.command.notification.mobile.exceptions.MobileNotificationEventStreamNotExistInEventStoreException
import com.syc.dashboard.command.notification.mobile.repository.jpa.MobileNotificationEventStoreRepository
...
@Service
class MobileNotificationEventStore @Autowired constructor(
    private val eventProducer: EventProducer,
    private val eventStoreRepository: MobileNotificationEventStoreRepository,
) : EventStore {
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = MobileNotificationEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw MobileNotificationEventStreamNotExistInEventStoreException(
                "Could not retrieve mobile notification event stream from the event store!",
            )
        }
