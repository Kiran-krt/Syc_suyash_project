...
import com.syc.dashboard.command.notification.email.exceptions.EmailNotificationEventStreamNotExistInEventStoreException
import com.syc.dashboard.command.notification.email.repository.jpa.EmailNotificationEventStoreRepository
...
@Service
class EmailNotificationEventStore @Autowired constructor(
    private val eventProducer: EventProducer,
    private val eventStoreRepository: EmailNotificationEventStoreRepository,
) : EventStore {
    ...
    override fun saveEvents(aggregateId: String, events: Iterable<BaseEvent>, expectedVersion: Int) {
        ...
        events
            .forEach {
                version++
                it.version = version
                val eventModel = EmailNotificationEventModel(
                    timeStamp = Date(),
    ...
    override fun getAggregateIds(): List<String> {
        ...
        if (eventStream.isEmpty()) {
            throw EmailNotificationEventStreamNotExistInEventStoreException(
                "Could not retrieve email notification event stream from the event store!",
            )
        }
