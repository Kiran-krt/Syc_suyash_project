...
abstract class TenantAggregateRoot(
    ...
    fun buildAggregateRoot(event: TenantBaseEvent) {
        if (!StringUtils.hasText(event.tenantId)) {
            throw TenantIdNotFoundInRequestHeaderException("Tenant ID not present in the event while building aggregate")
        }
