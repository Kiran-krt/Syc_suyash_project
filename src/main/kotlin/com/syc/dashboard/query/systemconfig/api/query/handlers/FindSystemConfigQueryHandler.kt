...
import com.syc.dashboard.query.systemconfig.dto.SystemConfigDto
...
class FindSystemConfigQueryHandler(
    ...
    override fun <T : BaseQuery> handle(query: T): Flux<out BaseDto> {
        query as FindSystemConfigQuery

        return systemConfigReactiveRepository
            .findByTenantId(tenantId = query.tenantId)
            .map { EntityDtoConversion.copyFromJson(it, SystemConfigDto::class.java) }
            .switchIfEmpty {
                log.warn("System Configuration not found for tenant ${query.tenantId}")
                Mono.just(SystemConfigDto())
            }
            .flux()
    }
}
