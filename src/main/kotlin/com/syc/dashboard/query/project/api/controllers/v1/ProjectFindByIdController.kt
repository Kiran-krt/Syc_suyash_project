...
@CrossOrigin
class ProjectFindByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/project/{id}/lookup")
    fun getProjectById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindProjectByIdQuery(id = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get project by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.PROJECT_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
