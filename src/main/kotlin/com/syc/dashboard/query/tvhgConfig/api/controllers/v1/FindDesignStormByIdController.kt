...
@CrossOrigin
class FindDesignStormByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/tvhgconfig/designstorm/lookup")
    fun getDesignStormById(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindDesignStormByIdQuery())

        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query))
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get design storm by id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.TVHG_CONFIG_DESIGN_STORM_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
