...
@CrossOrigin
class FindVehicleLogForEmployeeByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/vehiclelog/createdBy/{id}/vehicle/{vehicleId}/lookup/foremployee")
    fun getVehicleLogForEmployeeById(
        @PathVariable id: String,
        @PathVariable vehicleId: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindVehicleLogForEmployeeByIdQuery(createdBy = id, vehicleId = vehicleId))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get vehicle log for employee id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.VEHICLE_LOG_FOR_EMPLOYEE_ID_FAILED,
                        ),
                    ),
                )
        }
