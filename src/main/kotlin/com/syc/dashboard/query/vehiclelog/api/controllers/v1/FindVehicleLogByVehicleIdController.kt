...
@CrossOrigin
class FindVehicleLogByVehicleIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/vehicle/{id}/lookup/vehiclelog")
    fun getVehicleLog(
        @PathVariable("id")id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindVehicleLogByVehicleIdQuery(vehicleId = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get vehicle log by vehicle id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.VEHICLE_LOG_FIND_BY_VEHICLE_ID_FAILED,
                        ),
                    ),
                )
        }
