...
@CrossOrigin
class FindVehicleLogByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/vehiclelog/{id}/lookup")
    fun getVehicleLogById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindVehicleLogByIdQuery(id = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get vehicle log by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.VEHICLE_LOG_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
