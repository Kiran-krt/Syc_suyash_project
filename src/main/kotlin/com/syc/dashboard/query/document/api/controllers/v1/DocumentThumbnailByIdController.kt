...
@CrossOrigin
class DocumentThumbnailByIdController @Autowired constructor(
    private val queryDispatcher: QueryDispatcher,
    private val documentStorageService: DocumentStorageService,
) : RootController() {

    @GetMapping("/api/v1/document/{id}/thumbnail")
    fun getDocumentThumbnailById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): Mono<ResponseEntity<Mono<DataBuffer>>> {
        val query = buildQuery(DocumentThumbnailByIdQuery(id = id))
        ...
        return try {
            queryDispatcher.send(query = query).toMono()
                .map { documentDto ->
                    ...
                    Pair(
                        documentDto,
                        documentStorageService.thumbnail(tenantId, documentDto.cloudFilePath),
                    )
                }
                .map {
            ...
            val safeErrorMessage = "Failed to get document thumbnail by id request for id '$id'!"
