...
@CrossOrigin
class FindExpenseRowForEmployeeByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/expensereport/{id}/expenserows/lookup/foremployee")
    fun getExpenseRowForEmployeeById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindExpenseRowsForEmployeeByIdQuery(expenseReportId = id))

        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query))
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get expense rows for employee by id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.EXPENSE_REPORT_FOR_EMPLOYEE_BY_ID_FAILED,
                        ),
                    ),
                )
        }
