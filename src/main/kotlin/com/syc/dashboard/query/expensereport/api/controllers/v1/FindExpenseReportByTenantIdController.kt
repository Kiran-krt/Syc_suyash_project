...
@CrossOrigin
class FindExpenseReportByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/expensereport/lookup/all")
    fun getExpenseReportByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindExpenseReportByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get expense report by tenant id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.EXPENSE_REPORT_FIND_BY_TENANT_ID_FAILED,
                        ),
                    ),
                )
        }
