...
@CrossOrigin
class ExportExpenseReportToExcelByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/expensereport/{id}/export/toexcel")
    fun getExpenseReportById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(ExportExpenseReportToExcelByIdQuery(expenseReportId = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to export expense report to excel by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.EXPENSE_REPORT_EXPORT_TO_EXCEL_BY_ID_FAILED,
                        ),
                    ),
                )
        }
