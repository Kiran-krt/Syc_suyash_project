...
import com.syc.dashboard.query.employee.api.queries.FindEmployeeByStatusQuery
...
@CrossOrigin
class EmployeeFindByStatusController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/{id}/timesheet/pageable/lookup/bystatus/{status}")
    fun getEmployeeByStatus(
        @PathVariable id: String,
        @PathVariable status: TimesheetStatusEnum,
        query: FindEmployeeByStatusQuery,
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.EMPLOYEE_FIND_BY_STATUS_FAILED,
                        ),
                    ),
                )
        }
