...
import com.syc.dashboard.query.employee.api.queries.FindEmployeeByUserIdQuery
...
@CrossOrigin
class EmployeeFindByUserIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/{id}/lookup/withmanager")
    fun searchEmployeeByTenantId(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindEmployeeByUserIdQuery(id = id))
