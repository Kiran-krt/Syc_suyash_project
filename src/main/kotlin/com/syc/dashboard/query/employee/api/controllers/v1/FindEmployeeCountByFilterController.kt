...
import com.syc.dashboard.query.employee.api.queries.FindEmployeeCountByFilterQuery
...
@CrossOrigin
class FindEmployeeCountByFilterController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/count/search/all")
    fun getEmployeeCountyByStatus(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
        query: FindEmployeeCountByFilterQuery,
    ): ResponseEntity<Mono<out BaseDto>> {
        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query).toMono())
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get employee count by status request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.EMPLOYEE_COUNT_FIND_BY_STATUS_FAILED,
                        ),
                    ),
                )
        }
