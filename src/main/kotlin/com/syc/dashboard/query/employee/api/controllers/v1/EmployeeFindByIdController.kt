...
import com.syc.dashboard.query.employee.api.queries.FindEmployeeByIdQuery
...
@CrossOrigin
class EmployeeFindByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/{id}/lookup")
    fun getEmployeeById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindEmployeeByIdQuery(id = id))
