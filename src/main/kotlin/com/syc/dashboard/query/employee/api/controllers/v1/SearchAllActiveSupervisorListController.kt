...
@CrossOrigin
class SearchAllActiveSupervisorListController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/lookup/all/active/supervisor")
    fun getAllActiveSupervisorList(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(SearchAllActiveSupervisorListQuery())

        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query))
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get active supervisorlist by filter request!"
