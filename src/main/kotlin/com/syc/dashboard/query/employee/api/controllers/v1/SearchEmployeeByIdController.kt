...
import com.syc.dashboard.query.employee.api.queries.SearchEmployeeByIdQuery
...
@CrossOrigin
class SearchEmployeeByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/employee/lookup/formanager/{id}")
    fun searchAllEmployee(
        @PathVariable id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(SearchEmployeeByIdQuery(managerId = id))
