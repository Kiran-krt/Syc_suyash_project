...
import com.syc.dashboard.query.jobcode.api.queries.FindCostCodeByJobCodeIdQuery
...
@CrossOrigin
class FindCostCodeByJobCodeIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/jobcode/{id}/costcode/lookup/all")
    fun getCostCodeById(
        @PathVariable id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindCostCodeByJobCodeIdQuery(jobCodeId = id))
