...
import com.syc.dashboard.query.jobcode.api.queries.FindJobCodeByIdQuery
...
@CrossOrigin
class JobCodeFindByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/jobcode/{id}/lookup")
    fun getJobCodeById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindJobCodeByIdQuery(id = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get job code by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.JOBCODE_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
