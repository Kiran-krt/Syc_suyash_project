...
@CrossOrigin
class FindAllJobCodeByWatcherIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/jobcode/lookup/all/bywatcher/{watcherId}")
    fun getAllJobCodeByWatcher(
        @PathVariable watcherId: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindAllJobCodeByWatcherQuery(watcherId = watcherId))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get job code by watcher request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.JOBCODE_FIND_BY_WATCHER_FAILED,
                        ),
                    ),
                )
        }
