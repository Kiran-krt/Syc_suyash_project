...
import com.syc.dashboard.query.jobcode.api.queries.FindJobCodeByCodeQuery
...
@CrossOrigin
class JobCodeFindByCodeController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/jobcode/lookup/bycode/{code}")
    fun getJobCodeByCode(
        @PathVariable code: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindJobCodeByCodeQuery(code = code))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get job code by code request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.JOBCODE_FIND_BY_CODE_FAILED,
                        ),
                    ),
                )
        }
