...
@CrossOrigin
class AppendixFindByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/projectreport/{id}/appendix/lookup")
    fun getAppendixById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindAppendixByIdQuery(id = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get appendix by id request!"
