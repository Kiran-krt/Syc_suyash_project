...
import com.syc.dashboard.query.projectreport.api.queries.FindAllProjectReportQuery
...
@CrossOrigin
class FindAllProjectReportController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/projectreport/lookup/all")
    fun getAllProjectReportDetails(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindAllProjectReportQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get project report list by filter request!!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.PROJECT_REPORT_NOT_FOUND,
                        ),
                    ),
                )
        }
