...
@CrossOrigin
class SearchAllUsersByFilterController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/admin/lookup/allusers/search/all")
    fun getAllUsersForAdmin(
        query: SearchAllUsersByFilterQuery,
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get all users by filter request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.ALL_USERS_FIND_BY_FILTER_FAILED,
                        ),
                    ),
                )
        }
