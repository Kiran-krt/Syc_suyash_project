...
@CrossOrigin
class AdminFindByEmailController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/admin/lookup/byemail/{email}")
    fun getAdminByEmail(
        @PathVariable("email") email: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindAdminByEmailQuery(email = email))

        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query).toMono())
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get admin by email request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.ADMIN_NOT_FOUND_WITH_EMAIL,
                        ),
                    ),
                )
        }
