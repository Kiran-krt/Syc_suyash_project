...
@CrossOrigin
class FindAllActiveVehicleInfoController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/vehicleinfo/lookup/all/active")
    fun getAllVehicleInfo(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindAllActiveVehicleInfoQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get vehicle info request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.VEHICLE_INFO_FIND_FAILED,
                        ),
                    ),
                )
        }
