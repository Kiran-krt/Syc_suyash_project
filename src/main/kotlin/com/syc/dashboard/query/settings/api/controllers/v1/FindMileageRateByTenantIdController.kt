...
@CrossOrigin
class FindMileageRateByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/mileagerateinfo/lookup")
    fun getMileageRateInfoByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindSettingsMileageRateByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get mileage rate by tenant id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.MILEAGE_RATE_FIND_BY_TENANT_ID_FAILED,
                        ),
                    ),
                )
        }
