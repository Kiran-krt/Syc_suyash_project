...
@CrossOrigin
class FindPayrollItemByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/payrollitem/{id}/lookup")
    fun getPayrollItemById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindPayrollItemByIdQuery(id = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get payroll item by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.PAYROLL_ITEM_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
