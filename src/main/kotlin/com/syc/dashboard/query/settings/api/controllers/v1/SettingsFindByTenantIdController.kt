...
@CrossOrigin
class SettingsFindByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/lookup")
    fun getSettingsByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindSettingsByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get settings by tenantId request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.SETTINGS_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
