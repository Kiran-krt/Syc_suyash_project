...
@CrossOrigin
class FindYearlyQuarterByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/yearlyquarterinfo/lookup")
    fun getYearlyQuarterInfoByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindSettingsYearlyQuarterByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get yearly quarter info by tenant id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.YEARLY_QUARTER_FIND_BY_TENANT_ID_FAILED,
                        ),
                    ),
                )
        }
