...
@CrossOrigin
class FindPayrollItemByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/settings/payrollitem/lookup/all")
    fun getPayrollItemByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindPayrollItemByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get payroll item by tenant id request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.PAYROLL_ITEM_FIND_BY_TENANT_ID_FAILED,
                        ),
                    ),
                )
        }
