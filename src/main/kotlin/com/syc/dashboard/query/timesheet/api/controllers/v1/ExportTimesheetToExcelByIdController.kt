...
@CrossOrigin
class ExportTimesheetToExcelByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/timesheet/{id}/export/toexcel")
    fun getTimesheetById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(ExportTimesheetToExelByIdQuery(timesheetId = id))
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to export timesheet to excel by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.TIMESHEET_ROW_EXPORT_TO_EXCEL_BY_ID_FAILED,
                        ),
                    ),
                )
        }
