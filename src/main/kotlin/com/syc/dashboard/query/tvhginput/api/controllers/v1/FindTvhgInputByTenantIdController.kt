...
@CrossOrigin
class FindTvhgInputByTenantIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/tvhginput/lookup/all")
    fun getTvhgInputByTenantId(
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Flux<out BaseDto>> {
        val query = buildQuery(FindTvhgInputByTenantIdQuery())
        ...
        return try {
            ...
            val safeErrorMessage = "Failed to get tvhg input by tenantId request!"
                ...
                .body(
                    Flux.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.TVHG_INPUT_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
