...
@CrossOrigin
class FindProjectInformationByIdController @Autowired constructor(
    val queryDispatcher: QueryDispatcher,
) : RootController() {

    @GetMapping("/api/v1/tvhginput/{id}/projectinformation/lookup")
    fun getProjectInformationById(
        @PathVariable("id") id: String,
        request: ServerHttpRequest,
        response: ServerHttpResponse,
    ): ResponseEntity<Mono<out BaseDto>> {
        val query = buildQuery(FindProjectInformationByIdQuery(id = id))

        return try {
            ResponseEntity.ok(queryDispatcher.send(query = query).toMono())
        } catch (e: Exception) {
            val safeErrorMessage = "Failed to get project information by id request!"
                ...
                .body(
                    Mono.just(
                        ErrorDto(
                            ...
                            errorCode = ErrorCodes.TVHG_INPUT_PROJECT_INFORMATION_FIND_BY_ID_FAILED,
                        ),
                    ),
                )
        }
